...
/* Deterministic signing with option to get back recid parameter, similar to RFC 6979. Differences are:
...
int uECC_sign_deterministic_recoverable(const uint8_t *private_key,
    ...
    filled:
        ...
        if (uECC_sign_with_k(private_key, message_hash, hash_size, T, recid, signature, curve)) {
            return 1;
        }
...
int uECC_sign_deterministic(const uint8_t *private_key,
    ...
    return uECC_sign_deterministic_recoverable(private_key,message_hash,hash_size,hash_context,0,signature,curve);
} 
