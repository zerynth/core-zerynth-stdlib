...
static int pk_hw_write_ec_pubkey( unsigned char **p, unsigned char *start)
{
    int ret;
    size_t len = 0;
    unsigned char buf[MBEDTLS_ECP_MAX_PT_LEN];
...
int mbedtls_pk_hw_write_pubkey( unsigned char **p, unsigned char *start, const mbedtls_pk_context *key)
{
    int ret;
    size_t len = 0;

    if( mbedtls_pk_get_type( key ) == MBEDTLS_PK_ECKEY )
        MBEDTLS_ASN1_CHK_ADD( len, pk_hw_write_ec_pubkey( p, start ) );
    else
        return( MBEDTLS_ERR_PK_FEATURE_UNAVAILABLE );
    ...
    if( mbedtls_pk_get_type( key ) == MBEDTLS_PK_ECKEY )
    {
        if (ZERYNTH_HWCRYPTO_HASSIGN()) {
            if (zerynth_hwcrypto_ecdsa_secp256r1_sign) {
                ...
                if( ( ret = mbedtls_oid_get_oid_by_ec_grp( MBEDTLS_ECP_DP_SECP256R1, &sec_oid, &sec_oid_len ) ) != 0 )
                    return( ret );

                MBEDTLS_ASN1_CHK_ADD( par_len, mbedtls_asn1_write_oid( &c, buf, sec_oid, sec_oid_len ) );
            }
            else {
                return MBEDTLS_ERR_PK_FEATURE_UNAVAILABLE;
            }
