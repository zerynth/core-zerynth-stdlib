...
get_record           = __ota_get_record
set_record           = __ota_set_record
find_bytecode_slot   = __ota_find_bc_slot
find_vm_slot         = __ota_find_vm_slot
erase_slot           = __ota_erase_slot
write_slot           = __ota_write_slot
checksum_slot        = __ota_checksum_slot
close_slot           = __ota_close_slot
restart              = __ota_restart


def accept():
    info = get_record()
    if info[0]:
        current_vm = info[1] # running vm index
        current_bc = info[4]
        working_vm = info[3] # last working vm
        working_bc = info[5]
    else:
        current_vm = 0 # running vm index
        current_bc = 0
        working_vm = 0 # last working vm
        working_bc = 0

    if working_bc != current_bc or working_vm!=current_vm:
        if not set_record((current_vm,current_vm,current_bc,current_bc)):
            raise RuntimeError
    return True

def attempt(bcslot,vmslot):
    info = get_record()
    if info[0]:
        current_vm = info[1] # running vm index
        current_bc = info[4]
    ...
    if not set_record((vmslot,working_vm,bcslot,working_bc)):
        raise RuntimeError
    return True
