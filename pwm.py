...
@native_c("_vbl_pwm_init",["csrc/vbl/vbl_pwm.c"],["VHAL_PWM"])
def _vbl_pwm_init():
    pass
...
.. function:: init(drvname)
    ...
    res = _pwm_drv
    _vbl_pwm_init()
    _pwm_drv = __driver(drvname)
    #_pwm_drv.__ctl__(DRV_CMD_INIT,drvname&0xff)
    __builtins__.__default_pwm = _pwm_drv
    return res

init(PWMD0)


def write(pin,period,pulse,time_unit=MILLIS,npulses=0):
    """
.. function:: write(pin, period, pulse, time_unit=MILLIS,npulses=0)
    ...
    if not _pwm_drv:
        return None;
    _pwm_drv.__ctl__(DRV_CMD_WRITE,pin,period,pulse,time_unit,npulses)
